      ...
      if (providerId) {
        ...
        for (const func of moduleServiceFuncs) {
          if (this.rpcFuncIds.includes(`${providerId}.${func}`)) {
            throw new Error(
              `rpcService func [${providerId}.${func}] is exists!`
            );
          }
  ...
  async loadSchedule() {
    ...
    for (const module of scheduleModules) {
      ...
      if (providerId) {
        ...
        const moduleDefinition: ScheduleInterface = await this.applicationContext.getAsync(
          providerId
        );
        let targetHost = moduleDefinition.targetHost;
        if (
          moduleDefinition.targetHost &&
        ...
        if (
          ...
          schedule.scheduleJob(
            rule,
            moduleDefinition.resolve.bind(moduleDefinition)
          );
    ...
    if (middlewares && middlewares.length) {
      for (const middleware of middlewares) {
        if (typeof middleware === "function") {
          ...
          const middlewareImpl: WebMiddleware = await this.applicationContext.getAsync(
            middleware
          );
